/**
 * UuidFeature — Auto-generated by Plugin Generator
 *
 * Template: converter
...
import type { UuidConfig, UuidState } from './types'
import { defaults } from './settings'
import { process, getStats } from './logic'

export class UuidFeature extends BaseFeature<UuidConfig, string, string> {
  private _toolState: UuidState

  constructor(context: FeatureContext<UuidConfig>) {
    super(context, defaults)
    this._toolState = { input: '', output: null, inputSize: 0, outputSize: null }
  }

  get toolState(): Readonly<UuidState> { return this._toolState }
  ...
  async initialize(): Promise<void> {
    ...
    this.context.logger.info('UuidFeature initialized')
  }
  ...
  // ── Core Logic ────────────────────────────────────────────────────
  async run(input: string, config: UuidConfig): Promise<string> {
  ...
  transform(input: string, _config: UuidConfig): string {
    const s = getStats(input)
    this._toolState.inputSize = s.size
    return input.trim()
  }

  preview(_input: string, _config: UuidConfig): string | null { return null }
