/**
 * XmlFeature — XML Formatter Feature
 *
...
import type { XmlConfig, XmlState } from './types'
import { defaults } from './settings'
import { process, getStats } from './logic'

export class XmlFeature extends BaseFeature<XmlConfig, string, string> {
  private _toolState: XmlState

  constructor(context: FeatureContext<XmlConfig>) {
    super(context, defaults)
    this._toolState = { input: '', output: null, inputSize: 0, outputSize: null }
  }

  get toolState(): Readonly<XmlState> { return this._toolState }
  ...
  async initialize(): Promise<void> {
    ...
    this.context.logger.info('XmlFeature initialized')
  }

  async activate(): Promise<void> {
    this.lifecycle.transition('active')
    this.context.notification.info('XML', 'Ready — Format, minify, and validate XML')
  }
  ...
  // ── Core Logic ────────────────────────────────────────────────────
  async run(input: string, config: XmlConfig): Promise<string> {
  ...
  transform(input: string, _config: XmlConfig): string {
    const s = getStats(input)
    this._toolState.inputSize = s.size
    return input.trim()
  }

  preview(_input: string, _config: XmlConfig): string | null { return null }
