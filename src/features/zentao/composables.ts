/**
 * Zentao Plugin — Vue Composable
 *
 * Bridges ZentaoFeature to Vue reactivity.
 * No Core/Registry/Service access.
 */

import { ref, computed } from 'vue'
import { createFeatureContext } from '@/sdk/feature'
import { ZentaoFeature } from './ZentaoFeature'
import { createToolbar } from './toolbar'
import { defaults } from './settings'
import type { ZentaoConfig } from './types'

export function useZentao() {
  // Context & Feature
  const context = createFeatureContext<ZentaoConfig>({
    id: 'zentao',
    name: 'Zentao',
  ...
  const feature = new ZentaoFeature(context)
