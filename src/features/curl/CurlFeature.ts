/**
 * CurlFeature — Auto-generated by Plugin Generator
 *
 * Template: network
...
import type { CurlConfig, CurlState } from './types'
import { defaults } from './settings'
import { process, getStats } from './logic'

export class CurlFeature extends BaseFeature<CurlConfig, string, string> {
  private _toolState: CurlState

  constructor(context: FeatureContext<CurlConfig>) {
    super(context, defaults)
    this._toolState = { input: '', output: null, inputSize: 0, outputSize: null }
  }

  get toolState(): Readonly<CurlState> { return this._toolState }
  ...
  async initialize(): Promise<void> {
    ...
    this.context.logger.info('CurlFeature initialized')
  }

  async activate(): Promise<void> {
    this.lifecycle.transition('active')
    this.context.notification.info('Curl', 'Ready — Network tool — HTTP request and response analysis')
  }
  ...
  // ── Core Logic ────────────────────────────────────────────────────
  async run(input: string, config: CurlConfig): Promise<string> {
  ...
  transform(input: string, _config: CurlConfig): string {
    const s = getStats(input)
    this._toolState.inputSize = s.size
    return input.trim()
  }

  preview(_input: string, _config: CurlConfig): string | null { return null }
