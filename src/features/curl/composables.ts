/**
 * Curl Plugin — Vue Composable
 *
 * Bridges CurlFeature to Vue reactivity.
...
import { CurlFeature } from './CurlFeature'
...
export function useCurl() {
  // Context & Feature
  const context = createFeatureContext<CurlConfig>({
    id: 'curl',
    name: 'Curl',
  ...
  const feature = new CurlFeature(context)
