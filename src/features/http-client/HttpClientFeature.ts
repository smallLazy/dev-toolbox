/**
 * HttpClientFeature — Auto-generated by Plugin Generator
 *
 * Template: network
...
import type { HttpClientConfig, HttpClientState } from './types'
import { defaults } from './settings'
import { process, getStats } from './logic'

export class HttpClientFeature extends BaseFeature<HttpClientConfig, string, string> {
  private _toolState: HttpClientState

  constructor(context: FeatureContext<HttpClientConfig>) {
    super(context, defaults)
    this._toolState = { input: '', output: null, inputSize: 0, outputSize: null }
  }

  get toolState(): Readonly<HttpClientState> { return this._toolState }
  ...
  async initialize(): Promise<void> {
    ...
    this.context.logger.info('HttpClientFeature initialized')
  }

  async activate(): Promise<void> {
    this.lifecycle.transition('active')
    this.context.notification.info('HttpClient', 'Ready — Network tool — HTTP request and response analysis')
  }
  ...
  // ── Core Logic ────────────────────────────────────────────────────
  async run(input: string, config: HttpClientConfig): Promise<string> {
  ...
  transform(input: string, _config: HttpClientConfig): string {
    const s = getStats(input)
    this._toolState.inputSize = s.size
    return input.trim()
  }

  preview(_input: string, _config: HttpClientConfig): string | null { return null }
