/**
 * ReviewFeature — Auto-generated by Plugin Generator
 *
 * Template: ai
...
import type { ReviewConfig, ReviewState } from './types'
import { defaults } from './settings'
import { process, getStats } from './logic'

export class ReviewFeature extends BaseFeature<ReviewConfig, string, string> {
  private _toolState: ReviewState

  constructor(context: FeatureContext<ReviewConfig>) {
    super(context, defaults)
    this._toolState = { input: '', output: null, inputSize: 0, outputSize: null }
  }

  get toolState(): Readonly<ReviewState> { return this._toolState }
  ...
  async initialize(): Promise<void> {
    ...
    this.context.logger.info('ReviewFeature initialized')
  }
  ...
  // ── Core Logic ────────────────────────────────────────────────────
  async run(input: string, config: ReviewConfig): Promise<string> {
  ...
  transform(input: string, _config: ReviewConfig): string {
    const s = getStats(input)
    this._toolState.inputSize = s.size
    return input.trim()
  }

  preview(_input: string, _config: ReviewConfig): string | null { return null }
