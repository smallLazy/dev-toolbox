...
const primaryAction = computed<ToolAction>(() => ({
  ...
  shortcut: 'Cmd Enter',
  ariaLabel: mode.value === 'encode' ? 'Encode input to URL encoding' : 'Decode URL-encoded input',
}))
      ...
      <ToolWorkspace layout="io">
        <template #input>
          <InputOutputPanel
            ...
            :invalid="!!error"
            :aria-label="mode === 'encode' ? 'Plain URL or text input' : 'Encoded URL input'"
          >
            <textarea
              ref="inputEl"
              v-model="input"
        ...
        <template #output>
          <InputOutputPanel
            ...
            :value="output ?? ''"
            readonly
            :placeholder="mode === 'encode' ? 'URL-encoded output will appear here.' : 'Decoded text output will appear here.'"
            :stats="output ? outputPanelStats : null"
            :aria-label="mode === 'encode' ? 'URL-encoded output' : 'Decoded text output'"
          />
        </template>
      </ToolWorkspace>
    </template>
