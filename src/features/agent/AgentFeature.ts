/**
 * AgentFeature — Auto-generated by Plugin Generator
 *
 * Template: ai
...
export class AgentFeature extends BaseFeature<AgentConfig, string, string> {
  private _toolState: AgentState

  constructor(context: FeatureContext<AgentConfig>) {
    super(context, defaults)
    this._toolState = { input: '', output: null, inputSize: 0, outputSize: null }
  }

  get toolState(): Readonly<AgentState> { return this._toolState }
  ...
  async initialize(): Promise<void> {
    ...
    this.context.logger.info('AgentFeature initialized')
  }
  ...
  // ── Core Logic ────────────────────────────────────────────────────
  async run(input: string, config: AgentConfig): Promise<string> {
  ...
  transform(input: string, _config: AgentConfig): string {
    const s = getStats(input)
    this._toolState.inputSize = s.size
    return input.trim()
  }

  preview(_input: string, _config: AgentConfig): string | null { return null }
