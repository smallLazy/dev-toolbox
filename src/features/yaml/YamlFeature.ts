/**
 * YamlFeature — Auto-generated by Plugin Generator
 *
 * Template: editor
...
import type { YamlConfig, YamlState } from './types'
import { defaults } from './settings'
import { process, getStats } from './logic'

export class YamlFeature extends BaseFeature<YamlConfig, string, string> {
  private _toolState: YamlState

  constructor(context: FeatureContext<YamlConfig>) {
    super(context, defaults)
    this._toolState = { input: '', output: null, inputSize: 0, outputSize: null }
  }

  get toolState(): Readonly<YamlState> { return this._toolState }
  ...
  async initialize(): Promise<void> {
    ...
    this.context.logger.info('YamlFeature initialized')
  }
  ...
  // ── Core Logic ────────────────────────────────────────────────────
  async run(input: string, config: YamlConfig): Promise<string> {
  ...
  transform(input: string, _config: YamlConfig): string {
    const s = getStats(input)
    this._toolState.inputSize = s.size
    return input.trim()
  }

  preview(_input: string, _config: YamlConfig): string | null { return null }
