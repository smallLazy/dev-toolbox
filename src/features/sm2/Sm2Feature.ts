/**
 * Sm2Feature — Auto-generated by Plugin Generator
 *
 * Template: transform
...
import type { Sm2Config, Sm2State } from './types'
import { defaults } from './settings'
import { process, getStats } from './logic'

export class Sm2Feature extends BaseFeature<Sm2Config, string, string> {
  private _toolState: Sm2State

  constructor(context: FeatureContext<Sm2Config>) {
    super(context, defaults)
    this._toolState = { input: '', output: null, inputSize: 0, outputSize: null }
  }

  get toolState(): Readonly<Sm2State> { return this._toolState }
  ...
  async initialize(): Promise<void> {
    ...
    this.context.logger.info('Sm2Feature initialized')
  }
  ...
  // ── Core Logic ────────────────────────────────────────────────────
  async run(input: string, config: Sm2Config): Promise<string> {
  ...
  transform(input: string, _config: Sm2Config): string {
    const s = getStats(input)
    this._toolState.inputSize = s.size
    return input.trim()
  }

  preview(_input: string, _config: Sm2Config): string | null { return null }
