/**
 * Sm4Feature — Auto-generated by Plugin Generator
 *
 * Template: transform
...
import type { Sm4Config, Sm4State } from './types'
import { defaults } from './settings'
import { process, getStats } from './logic'

export class Sm4Feature extends BaseFeature<Sm4Config, string, string> {
  private _toolState: Sm4State

  constructor(context: FeatureContext<Sm4Config>) {
    super(context, defaults)
    this._toolState = { input: '', output: null, inputSize: 0, outputSize: null }
  }

  get toolState(): Readonly<Sm4State> { return this._toolState }
  ...
  async initialize(): Promise<void> {
    ...
    this.context.logger.info('Sm4Feature initialized')
  }
  ...
  // ── Core Logic ────────────────────────────────────────────────────
  async run(input: string, config: Sm4Config): Promise<string> {
  ...
  transform(input: string, _config: Sm4Config): string {
    const s = getStats(input)
    this._toolState.inputSize = s.size
    return input.trim()
  }

  preview(_input: string, _config: Sm4Config): string | null { return null }
