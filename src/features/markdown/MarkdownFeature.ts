/**
 * MarkdownFeature — Auto-generated by Plugin Generator
 *
 * Template: editor
...
import type { MarkdownConfig, MarkdownState } from './types'
import { defaults } from './settings'
import { process, getStats } from './logic'

export class MarkdownFeature extends BaseFeature<MarkdownConfig, string, string> {
  private _toolState: MarkdownState

  constructor(context: FeatureContext<MarkdownConfig>) {
    super(context, defaults)
    this._toolState = { input: '', output: null, inputSize: 0, outputSize: null }
  }

  get toolState(): Readonly<MarkdownState> { return this._toolState }
  ...
  async initialize(): Promise<void> {
    ...
    this.context.logger.info('MarkdownFeature initialized')
  }

  async activate(): Promise<void> {
    this.lifecycle.transition('active')
    this.context.notification.info('Markdown', 'Ready — Editor tool — format, minify, and validate')
  }
  ...
  // ── Core Logic ────────────────────────────────────────────────────
  async run(input: string, config: MarkdownConfig): Promise<string> {
  ...
  transform(input: string, _config: MarkdownConfig): string {
    const s = getStats(input)
    this._toolState.inputSize = s.size
    return input.trim()
  }

  preview(_input: string, _config: MarkdownConfig): string | null { return null }
