/**
 * GithubFeature — Auto-generated by Plugin Generator
 *
 * Template: enterprise
...
import type { GithubConfig, GithubState } from './types'
import { defaults } from './settings'
import { process, getStats } from './logic'

export class GithubFeature extends BaseFeature<GithubConfig, string, string> {
  private _toolState: GithubState

  constructor(context: FeatureContext<GithubConfig>) {
    super(context, defaults)
    this._toolState = { input: '', output: null, inputSize: 0, outputSize: null }
  }

  get toolState(): Readonly<GithubState> { return this._toolState }
  ...
  async initialize(): Promise<void> {
    ...
    this.context.logger.info('GithubFeature initialized')
  }

  async activate(): Promise<void> {
    this.lifecycle.transition('active')
    this.context.notification.info('Github', 'Ready — Enterprise tool — external service integration')
  }
  ...
  // ── Core Logic ────────────────────────────────────────────────────
  async run(input: string, config: GithubConfig): Promise<string> {
  ...
  transform(input: string, _config: GithubConfig): string {
    const s = getStats(input)
    this._toolState.inputSize = s.size
    return input.trim()
  }

  preview(_input: string, _config: GithubConfig): string | null { return null }
