/**
 * SentryFeature — Auto-generated by Plugin Generator
 *
 * Template: enterprise
...
import type { SentryConfig, SentryState } from './types'
import { defaults } from './settings'
import { process, getStats } from './logic'

export class SentryFeature extends BaseFeature<SentryConfig, string, string> {
  private _toolState: SentryState

  constructor(context: FeatureContext<SentryConfig>) {
    super(context, defaults)
    this._toolState = { input: '', output: null, inputSize: 0, outputSize: null }
  }

  get toolState(): Readonly<SentryState> { return this._toolState }
  ...
  async initialize(): Promise<void> {
    ...
    this.context.logger.info('SentryFeature initialized')
  }

  async activate(): Promise<void> {
    this.lifecycle.transition('active')
    this.context.notification.info('Sentry', 'Ready — Enterprise tool — external service integration')
  }
  ...
  // ── Core Logic ────────────────────────────────────────────────────
  async run(input: string, config: SentryConfig): Promise<string> {
  ...
  transform(input: string, _config: SentryConfig): string {
    const s = getStats(input)
    this._toolState.inputSize = s.size
    return input.trim()
  }

  preview(_input: string, _config: SentryConfig): string | null { return null }
