...
> Auto-generated by Plugin Generator. Template: `inspector`
...
```
features/regex/
├── RegexFeature.ts    ← Feature class (extends BaseFeature)
...
├── RegexView.vue       ← Main view
├── index.ts              ← Public API
└── __tests__/
    └── logic.test.ts     ← Unit tests

plugins/regex.plugin.ts ← Plugin manifest
```
