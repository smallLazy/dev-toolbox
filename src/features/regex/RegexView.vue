...
import { useRegex } from './composables'
...
const { input, output, error, loading, toolbar, execute, init, dispose } = useRegex()
...
<template>
  <ToolLayout
    title="Regex"
    description="Test regular expressions and display matches."
      ...
      <ToolWorkspace layout="io">
        ...
        <template #output>
          <InputOutputPanel
            ...
            :value="output ?? ''"
            readonly
            placeholder="Regex match results will appear here."
            :stats="output ? { chars: output.length } : null"
            aria-label="Regex output"
          />
        </template>
      </ToolWorkspace>
    </template>
