/**
 * RegexFeature — Auto-generated by Plugin Generator
 *
 * Template: inspector
...
import type { RegexConfig, RegexState } from './types'
import { defaults } from './settings'
import { process, getStats } from './logic'

export class RegexFeature extends BaseFeature<RegexConfig, string, string> {
  private _toolState: RegexState

  constructor(context: FeatureContext<RegexConfig>) {
    super(context, defaults)
    this._toolState = { input: '', output: null, inputSize: 0, outputSize: null }
  }

  get toolState(): Readonly<RegexState> { return this._toolState }
  ...
  async initialize(): Promise<void> {
    ...
    this.context.logger.info('RegexFeature initialized')
  }
  ...
  // ── Core Logic ────────────────────────────────────────────────────
  async run(input: string, config: RegexConfig): Promise<string> {
  ...
  transform(input: string, _config: RegexConfig): string {
    const s = getStats(input)
    this._toolState.inputSize = s.size
    return input.trim()
  }

  preview(_input: string, _config: RegexConfig): string | null { return null }
