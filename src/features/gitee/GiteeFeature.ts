/**
 * GiteeFeature — Auto-generated by Plugin Generator
 *
 * Template: enterprise
...
import type { GiteeConfig, GiteeState } from './types'
import { defaults } from './settings'
import { process, getStats } from './logic'

export class GiteeFeature extends BaseFeature<GiteeConfig, string, string> {
  private _toolState: GiteeState

  constructor(context: FeatureContext<GiteeConfig>) {
    super(context, defaults)
    this._toolState = { input: '', output: null, inputSize: 0, outputSize: null }
  }

  get toolState(): Readonly<GiteeState> { return this._toolState }
  ...
  async initialize(): Promise<void> {
    ...
    this.context.logger.info('GiteeFeature initialized')
  }

  async activate(): Promise<void> {
    this.lifecycle.transition('active')
    this.context.notification.info('Gitee', 'Ready — Enterprise tool — external service integration')
  }
  ...
  // ── Core Logic ────────────────────────────────────────────────────
  async run(input: string, config: GiteeConfig): Promise<string> {
  ...
  transform(input: string, _config: GiteeConfig): string {
    const s = getStats(input)
    this._toolState.inputSize = s.size
    return input.trim()
  }

  preview(_input: string, _config: GiteeConfig): string | null { return null }
