/**
 * PromptFeature — Auto-generated by Plugin Generator
 *
 * Template: ai
...
import type { PromptConfig, PromptState } from './types'
import { defaults } from './settings'
import { process, getStats } from './logic'

export class PromptFeature extends BaseFeature<PromptConfig, string, string> {
  private _toolState: PromptState

  constructor(context: FeatureContext<PromptConfig>) {
    super(context, defaults)
    this._toolState = { input: '', output: null, inputSize: 0, outputSize: null }
  }

  get toolState(): Readonly<PromptState> { return this._toolState }
  ...
  async initialize(): Promise<void> {
    ...
    this.context.logger.info('PromptFeature initialized')
  }

  async activate(): Promise<void> {
    this.lifecycle.transition('active')
    this.context.notification.info('Prompt', 'Ready — AI tools — chat, completion, and generation')
  }
  ...
  // ── Core Logic ────────────────────────────────────────────────────
  async run(input: string, config: PromptConfig): Promise<string> {
  ...
  transform(input: string, _config: PromptConfig): string {
    const s = getStats(input)
    this._toolState.inputSize = s.size
    return input.trim()
  }

  preview(_input: string, _config: PromptConfig): string | null { return null }
