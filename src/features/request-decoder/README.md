...
```
features/request-decoder/
├── RequestDecoderFeature.ts    ← Feature class (extends BaseFeature)
...
├── RequestDecoderView.vue       ← Main view
├── index.ts              ← Public API
└── __tests__/
    └── logic.test.ts     ← Unit tests

plugins/request-decoder.plugin.ts ← Plugin manifest
```
