/**
 * RequestDecoder Plugin — Vue Composable
 *
 * Bridges RequestDecoderFeature to Vue reactivity.
...
import { RequestDecoderFeature } from './RequestDecoderFeature'
import { createToolbar } from './toolbar'
import { defaults } from './settings'
import type { RequestDecoderConfig } from './types'

export function useRequestDecoder() {
  // Context & Feature
  const context = createFeatureContext<RequestDecoderConfig>({
    id: 'request-decoder',
    name: 'RequestDecoder',
  ...
  const feature = new RequestDecoderFeature(context)
