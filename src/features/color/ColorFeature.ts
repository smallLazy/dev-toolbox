/**
 * ColorFeature — Auto-generated by Plugin Generator
 *
 * Template: converter
...
import type { ColorConfig, ColorState } from './types'
import { defaults } from './settings'
import { process, getStats } from './logic'

export class ColorFeature extends BaseFeature<ColorConfig, string, string> {
  private _toolState: ColorState

  constructor(context: FeatureContext<ColorConfig>) {
    super(context, defaults)
    this._toolState = { input: '', output: null, inputSize: 0, outputSize: null }
  }

  get toolState(): Readonly<ColorState> { return this._toolState }
  ...
  async initialize(): Promise<void> {
    ...
    this.context.logger.info('ColorFeature initialized')
  }

  async activate(): Promise<void> {
    this.lifecycle.transition('active')
    this.context.notification.info('Color', 'Ready — 格式互转工具 — 两种格式之间互相转换')
  }
  ...
  // ── Core Logic ────────────────────────────────────────────────────
  async run(input: string, config: ColorConfig): Promise<string> {
  ...
  transform(input: string, _config: ColorConfig): string {
    const s = getStats(input)
    this._toolState.inputSize = s.size
    return input.trim()
  }

  preview(_input: string, _config: ColorConfig): string | null { return null }
