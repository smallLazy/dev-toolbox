/**
 * GraphqlFeature — Auto-generated by Plugin Generator
 *
 * Template: network
...
import type { GraphqlConfig, GraphqlState } from './types'
import { defaults } from './settings'
import { process, getStats } from './logic'

export class GraphqlFeature extends BaseFeature<GraphqlConfig, string, string> {
  private _toolState: GraphqlState

  constructor(context: FeatureContext<GraphqlConfig>) {
    super(context, defaults)
    this._toolState = { input: '', output: null, inputSize: 0, outputSize: null }
  }

  get toolState(): Readonly<GraphqlState> { return this._toolState }
  ...
  async initialize(): Promise<void> {
    ...
    this.context.logger.info('GraphqlFeature initialized')
  }
  ...
  // ── Core Logic ────────────────────────────────────────────────────
  async run(input: string, config: GraphqlConfig): Promise<string> {
  ...
  transform(input: string, _config: GraphqlConfig): string {
    const s = getStats(input)
    this._toolState.inputSize = s.size
    return input.trim()
  }

  preview(_input: string, _config: GraphqlConfig): string | null { return null }
