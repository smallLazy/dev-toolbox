/**
 * TranslateFeature — Auto-generated by Plugin Generator
 *
 * Template: ai
...
import type { TranslateConfig, TranslateState } from './types'
import { defaults } from './settings'
import { process, getStats } from './logic'

export class TranslateFeature extends BaseFeature<TranslateConfig, string, string> {
  private _toolState: TranslateState

  constructor(context: FeatureContext<TranslateConfig>) {
    super(context, defaults)
    this._toolState = { input: '', output: null, inputSize: 0, outputSize: null }
  }

  get toolState(): Readonly<TranslateState> { return this._toolState }
  ...
  async initialize(): Promise<void> {
    ...
    this.context.logger.info('TranslateFeature initialized')
  }
  ...
  // ── Core Logic ────────────────────────────────────────────────────
  async run(input: string, config: TranslateConfig): Promise<string> {
  ...
  transform(input: string, _config: TranslateConfig): string {
    const s = getStats(input)
    this._toolState.inputSize = s.size
    return input.trim()
  }

  preview(_input: string, _config: TranslateConfig): string | null { return null }
