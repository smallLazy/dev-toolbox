/**
 * Wecom Plugin — Vue Composable
 *
 * Bridges WecomFeature to Vue reactivity.
...
import { WecomFeature } from './WecomFeature'
...
export function useWecom() {
  // Context & Feature
  const context = createFeatureContext<WecomConfig>({
    id: 'wecom',
    name: 'Wecom',
  ...
  const feature = new WecomFeature(context)
