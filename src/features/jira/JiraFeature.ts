/**
 * JiraFeature — Auto-generated by Plugin Generator
 *
 * Template: enterprise
...
import type { JiraConfig, JiraState } from './types'
import { defaults } from './settings'
import { process, getStats } from './logic'

export class JiraFeature extends BaseFeature<JiraConfig, string, string> {
  private _toolState: JiraState

  constructor(context: FeatureContext<JiraConfig>) {
    super(context, defaults)
    this._toolState = { input: '', output: null, inputSize: 0, outputSize: null }
  }

  get toolState(): Readonly<JiraState> { return this._toolState }
  ...
  async initialize(): Promise<void> {
    ...
    this.context.logger.info('JiraFeature initialized')
  }

  async activate(): Promise<void> {
    this.lifecycle.transition('active')
    this.context.notification.info('Jira', 'Ready — Enterprise tool — external service integration')
  }
  ...
  // ── Core Logic ────────────────────────────────────────────────────
  async run(input: string, config: JiraConfig): Promise<string> {
  ...
  transform(input: string, _config: JiraConfig): string {
    const s = getStats(input)
    this._toolState.inputSize = s.size
    return input.trim()
  }

  preview(_input: string, _config: JiraConfig): string | null { return null }
