/**
 * ExplainFeature — Auto-generated by Plugin Generator
 *
 * Template: ai
...
import type { ExplainConfig, ExplainState } from './types'
import { defaults } from './settings'
import { process, getStats } from './logic'

export class ExplainFeature extends BaseFeature<ExplainConfig, string, string> {
  private _toolState: ExplainState

  constructor(context: FeatureContext<ExplainConfig>) {
    super(context, defaults)
    this._toolState = { input: '', output: null, inputSize: 0, outputSize: null }
  }

  get toolState(): Readonly<ExplainState> { return this._toolState }
  ...
  async initialize(): Promise<void> {
    ...
    this.context.logger.info('ExplainFeature initialized')
  }

  async activate(): Promise<void> {
    this.lifecycle.transition('active')
    this.context.notification.info('Explain', 'Ready — AI tools — chat, completion, and generation')
  }
  ...
  // ── Core Logic ────────────────────────────────────────────────────
  async run(input: string, config: ExplainConfig): Promise<string> {
  ...
  transform(input: string, _config: ExplainConfig): string {
    const s = getStats(input)
    this._toolState.inputSize = s.size
    return input.trim()
  }

  preview(_input: string, _config: ExplainConfig): string | null { return null }
