/**
 * RsaFeature — Auto-generated by Plugin Generator
 *
 * Template: transform
...
import type { RsaConfig, RsaState } from './types'
import { defaults } from './settings'
import { process, getStats } from './logic'

export class RsaFeature extends BaseFeature<RsaConfig, string, string> {
  private _toolState: RsaState

  constructor(context: FeatureContext<RsaConfig>) {
    super(context, defaults)
    this._toolState = { input: '', output: null, inputSize: 0, outputSize: null }
  }

  get toolState(): Readonly<RsaState> { return this._toolState }
  ...
  async initialize(): Promise<void> {
    ...
    this.context.logger.info('RsaFeature initialized')
  }
  ...
  // ── Core Logic ────────────────────────────────────────────────────
  async run(input: string, config: RsaConfig): Promise<string> {
  ...
  transform(input: string, _config: RsaConfig): string {
    const s = getStats(input)
    this._toolState.inputSize = s.size
    return input.trim()
  }

  preview(_input: string, _config: RsaConfig): string | null { return null }
