/**
 * QrcodeFeature — Auto-generated by Plugin Generator
 *
 * Template: viewer
...
import type { QrcodeConfig, QrcodeState } from './types'
import { defaults } from './settings'
import { process, getStats } from './logic'

export class QrcodeFeature extends BaseFeature<QrcodeConfig, string, string> {
  private _toolState: QrcodeState

  constructor(context: FeatureContext<QrcodeConfig>) {
    super(context, defaults)
    this._toolState = { input: '', output: null, inputSize: 0, outputSize: null }
  }

  get toolState(): Readonly<QrcodeState> { return this._toolState }
  ...
  async initialize(): Promise<void> {
    ...
    this.context.logger.info('QrcodeFeature initialized')
  }
  ...
  // ── Core Logic ────────────────────────────────────────────────────
  async run(input: string, config: QrcodeConfig): Promise<string> {
  ...
  transform(input: string, _config: QrcodeConfig): string {
    const s = getStats(input)
    this._toolState.inputSize = s.size
    return input.trim()
  }

  preview(_input: string, _config: QrcodeConfig): string | null { return null }
