/**
 * Sm3Feature — Auto-generated by Plugin Generator
 *
 * Template: transform
...
import type { Sm3Config, Sm3State } from './types'
import { defaults } from './settings'
import { process, getStats } from './logic'

export class Sm3Feature extends BaseFeature<Sm3Config, string, string> {
  private _toolState: Sm3State

  constructor(context: FeatureContext<Sm3Config>) {
    super(context, defaults)
    this._toolState = { input: '', output: null, inputSize: 0, outputSize: null }
  }

  get toolState(): Readonly<Sm3State> { return this._toolState }
  ...
  async initialize(): Promise<void> {
    ...
    this.context.logger.info('Sm3Feature initialized')
  }

  async activate(): Promise<void> {
    this.lifecycle.transition('active')
    this.context.notification.info('Sm3', 'Ready — Data conversion tool')
  }
  ...
  // ── Core Logic ────────────────────────────────────────────────────
  async run(input: string, config: Sm3Config): Promise<string> {
  ...
  transform(input: string, _config: Sm3Config): string {
    const s = getStats(input)
    this._toolState.inputSize = s.size
    return input.trim()
  }

  preview(_input: string, _config: Sm3Config): string | null { return null }
