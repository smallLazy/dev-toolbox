...
async function execute() {
  ...
  try {
    ...
    if (algorithm.value === "md5") {
      // MD5 via Web Crypto is not available, use a simple implementation
...
// Simple MD5 implementation since Web Crypto doesn't support it
async function md5Hash(data: Uint8Array): Promise<string> {
  // Use a well-known MD5 approach
...
<template>
  <div class="tool-panel">
    <h2 class="tool-title">🔑 MD5 / SHA256</h2>
    <p class="tool-desc">计算文本的 MD5 或 SHA-256 哈希值</p>
    ...
    <div class="form-group full-width">
      <label>输入</label>
      <textarea v-model="input" rows="6" placeholder="输入要计算哈希的文本..." />
    </div>
...
.hash-output {
  ...
  border-radius: 6px; font-size: 14px; font-family: "Menlo", "Monaco", "Courier New", monospace;
  color: #1e1e2e; word-break: break-all;
}
.error-msg {
